  ...
  $scope.addAppointment = function() {
    ...
    .then(function(response){
      if(response.data.data) {
        ...
        $scope.appointments.push(response.data.data);
        $scope.dataArr.push({ title: res.title, start: new Date(res.startDate), end: new Date(res.endDate), text: res.description });
  ...
  function calender()  {
    ...
    if($scope.appointments) {
      for(var i=0; i<$scope.appointments.length; i++) {
            data.push({ title: $scope.appointments[i].title, start: new Date($scope.appointments[i].startDate), end: new Date($scope.appointments[i].endDate), text: $scope.appointments[i].description });
      }
      $scope.dataArr = data;
      ...
      setTimeout(function(){
          $('#holder').load({ "data[]": $scope.appointments })
      },1000)
    } else {
      data.sort(function(a,b) { return (+a.start) - (+b.start); });
      $('#holder').calendar({
